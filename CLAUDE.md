...
./gradlew test --tests "com.estudios.demosafkaspring.ApplicationTests"
...
**Infrastructure:** The app relies on Docker Compose for Kafka. The `compose.yaml` is currently empty — a Kafka broker service must be added before `bootRun` will work. Spring Boot's Docker Compose integration will automatically start the services in `compose.yaml` when running locally.
